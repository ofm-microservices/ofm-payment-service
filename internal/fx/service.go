...
func ProvidePaymentService(writeRepo domain.PaymentIntentRepository, webhooks domain.WebhookRepository, read domain.PaymentIntentReadRepository, accounts domain.ConnectAccountRepository, releases domain.PaymentReleaseRepository, broker application.EventBroker, stripe application.StripeConnectGateway, lg logging.Logger) (application.Service, error) {
	return application.New(writeRepo, webhooks, read, accounts, releases, broker, stripe, lg)
}
